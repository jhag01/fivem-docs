...
local newState = exports['txLogin']:toggleDuty(source, true)
```

<Callout type="info">
  If a [cooldown](/txlogin/modules/#cooldown-servercooldownlua) is configured, it applies to every call to this function, including calls made through this export — there's no separate "forced" bypass path.
</Callout>
...
Requires [`Settings.DutyTracking`](/txlogin/configuration/#duty-tracking-and-cooldown) to be enabled (it is by default).
