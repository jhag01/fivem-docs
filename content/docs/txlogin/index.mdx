...
txLogin is a standalone FiveM resource that bridges txAdmin authentication with a togglable "Duty" status. Staff can toggle their admin presence on/off, which is exposed to other resources via state bags — useful for incognito modes, staff tracking, or gating the txAdmin menu behind duty status.

<ResourceLinks repo="txLogin" />
...
<Cards>
  <Card title="Installation" href="/txlogin/installation/" description="Get txLogin running, including the required txAdmin menu edit" />
  <Card title="Configuration" href="/txlogin/configuration/" description="Walk through settings.lua" />
  <Card title="Integration" href="/txlogin/integration/" description="State bags and reactive handlers for other resources" />
</Cards>
